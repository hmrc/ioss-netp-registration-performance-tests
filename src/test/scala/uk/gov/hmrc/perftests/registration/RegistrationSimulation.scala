...
class RegistrationSimulation extends PerformanceTestRunner {
  ...
  val deletePendingRegistrationUrl: String = s"$netpRegistrationBaseUrl/pay-clients-vat-on-eu-sales/register-new-ioss-client/test-only/delete-pending-registrations"

  before {
    println("Clearing existing pending registrations from the database")
    clearAll(deletePendingRegistrationUrl)
  ...
  setup("registrationUkVrn", "IOSS NETP Registration Journey - UK Based VRN") withRequests
    (
      ...
      getClientVatNumber,
      postClientVatNumber,
      getConfirmTaxDetails,
      postConfirmTaxDetails,
      getHaveTradingName,
      postHaveTradingName,
      getTradingName(1),
      postTradingName(1, "1st trading name"),
      getAddTradingName,
      postAddTradingName(true, Some(2)),
      getTradingName(2),
      postTradingName(2, "another-trading-name"),
      getAddTradingName,
  ...
  setup("registrationNonUk", "IOSS NETP Registration Journey - Non-UK based") withRequests
    (
      ...
      postHaveTradingName,
      getTradingName(1),
      postTradingName(1, "trading name"),
      getAddTradingName,
      postAddTradingName(false, None),
      postPreviousOss(1),
      getPreviousCountry(1),
